...
export async function createJob(input: CreateJobInput): Promise<Job> {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    const newRow = {
      ...
      responsibilities: input.responsibilities?.length ? input.responsibilities : null,
      requirements: input.requirements?.length ? input.requirements : null,
      nicetohave: input.niceToHave?.length ? input.niceToHave : null,
      portfoliorequirement: input.portfolioRequirement || null,
      benefits_remote: input.benefitsRemote?.length ? input.benefitsRemote : null,
      benefits_inperson: input.benefitsInPerson?.length ? input.benefitsInPerson : null,
      workculture: input.workCulture?.length ? input.workCulture : null,
